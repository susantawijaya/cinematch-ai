...
export default function SynkoraDashboard({ csrfToken, initialQuery, allAssets, chatMessages, googleToken, apiKey, clientId, error, success }) {
  ...
  // 🔥 FASE 4: SEMANTIC SEARCH ENABLED
  // Sekarang kolom pencarian juga mendeteksi hashtag/semantic tags
  ...
  // Fungsi pengelola modal tetap sama...
  ...
  const handleChatSubmit = async (e) => {
    ...
    progressIntervalRef.current = setInterval(() => {
      setProgressStage(prev => {
        if (prev < 5) {
          ...
          if (next === 4) setProgressStatus('Gemini sedang menilai Vibe Score & menyusun Semantic Tags...');
    ...
    try {
      ...
      pollingIntervalRef.current = setInterval(async () => {
        ...
        } catch (pollErr) { console.error("Gagal mengecek status:", pollErr); }
      }, 3000);
      ...
      setModal({
        ...
        message: 'Gagal mengirim instruksi ke server antrean.',
        confirmText: 'Tutup', isDestructive: false
      });
    }
  };
  ...
  // 🔥 Helper untuk merender Vibe Score
    ...
    return (
      <div className="flex items-center gap-1.5 mt-2 bg-white/60 p-1.5 rounded-lg border border-white/40 shadow-sm w-max">
        ...
        <div className="w-20 h-1.5 bg-zinc-200 rounded-full overflow-hidden">
          <div 
            className={`h-full rounded-full ${isHot ? 'bg-gradient-to-r from-orange-400 to-red-500' : isCold ? 'bg-gradient-to-r from-blue-300 to-cyan-500' : 'bg-gradient-to-r from-indigo-400 to-purple-500'}`} 
            style={{ width: `${score}%` }}
          ></div>
        </div>
        <span className={`text-[10px] font-black ${isHot ? 'text-red-500' : isCold ? 'text-cyan-600' : 'text-indigo-600'}`}>
          {score} {isHot && '🔥'} {isCold && '❄️'}
        </span>
      </div>
    );
  };

  // 🔥 Helper untuk merender Semantic Tags
  const renderTags = (tags) => {
    if (!tags) return null;
    let tagArray = [];
    if (typeof tags === 'string') {
      try { tagArray = JSON.parse(tags); } catch (e) { return null; }
    ...
    return (
      <div className="flex flex-wrap gap-1 mt-2">
        {tagArray.map((tag, idx) => (
          <span key={idx} className="text-[9px] font-bold bg-indigo-50 text-indigo-600 border border-indigo-100 px-1.5 py-0.5 rounded-md">
            {tag}
          </span>
        ))}
      </div>
                ...
                <>
                  ...
                  {Object.keys(subFoldersMap).length > 0 && (
                    <div className="border-t border-zinc-300/50 pt-6">
                      <div className="flex items-center gap-2 cursor-pointer group w-max mb-4 select-none" onClick={() => toggleSection('all_subfolders')}>
                                    ...
                                    {subFoldersMap[folderName].map((clip, idx) => (
                                      <div key={idx} className="bg-[#F0EEE6] border border-white/60 p-4 rounded-xl flex flex-col justify-between shadow-sm">
                                        ...
                                        <div>
                                          ...
                                          {/* 🔥 TAMPILAN FASE 4: VIBE & SEMANTIC TAGS */}
